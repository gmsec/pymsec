...
def StartService(serviceName,addr):
    global service_name
    global server_addr
...
def lease_server(inc, schedule):
    ...
    # 进行租期约需
    path = service_name
    value = {"Op":0,"Addr":server_addr,"Metadata":int(time.time())}
    key = path + "@" + json.dumps(value) 
    if is_lease:
        print("------------续约---------", time.time())
        lease.refresh()  # 刷新续约时间, 如果直接put,会触发监听变化,续约则不会。
    else:
        client.put(key, json.dumps(value), lease=lease)
        is_lease = True
...
if __name__ == '__main__':
    StartService(service_name,server_addr)
